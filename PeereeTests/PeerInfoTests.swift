...
class PeerInfoTests: XCTestCase {
	func testEncoding() throws {
		...
		let peerInfo = PeerInfo(peerID: PeerID(), publicKey: keyPair.publicKey, nickname: "The Hanger", gender: .male, age: nil, hasPicture: false)
		let encoder = JSONEncoder()
		let data = try encoder.encode(peerInfo)
		let decoder = JSONDecoder()
		let decodedPeerInfo = try decoder.decode(PeerInfo.self, from: data)
		XCTAssertEqual(peerInfo, decodedPeerInfo)
		XCTAssertEqual(peerInfo.hashValue, decodedPeerInfo.hashValue)
		XCTAssertEqual(peerInfo.age, decodedPeerInfo.age)
		XCTAssertEqual(peerInfo.biography, decodedPeerInfo.biography)
